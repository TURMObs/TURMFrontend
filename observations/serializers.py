...
class ExoplanetObservationSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        target, created = CelestialTarget.objects.get_or_create(name=target_name, defaults= {
            'coordinates': target_data.get('coordinates')
        })
        observation = ExoplanetObservation.objects.create(target=target, **validated_data)
        return observation
