...
from observation_data.models import Observatory, ExposureSettings, ObservatoryExposureSettings, ObservationType, Filter
...
class Command(BaseCommand):
    ...
    def populate_exposure_settings():
        ...
        ObservatoryExposureSettings.objects.get_or_create(
            ...
            exposure_settings=ExposureSettings.objects.get(gain=2750, offset=0, binning=1, subFrame=1.0),
            observation_type=ObservationType.IMAGING
        )

        for observation_type in [ObservationType.EXOPLANET, ObservationType.VARIABLE, ObservationType.MONITORING]:
            ObservatoryExposureSettings.objects.get_or_create(
                observatory=Observatory.objects.get(name="TURMX"),
                exposure_settings=ExposureSettings.objects.get(gain=0, offset=50, binning=1, subFrame=0.25),
                observation_type=observation_type

            )
            ObservatoryExposureSettings.objects.get_or_create(
                observatory=Observatory.objects.get(name="TURMX2"),
                exposure_settings=ExposureSettings.objects.get(gain=0, offset=0, binning=1, subFrame=0.5),
                observation_type=observation_type
            )

    @staticmethod
    def populate_filters():
        for filter_type in [Filter.FilterType.LUMINANCE, Filter.FilterType.RED, Filter.FilterType.GREEN, Filter.FilterType.BLUE]:
        ...
        for filter_type in [Filter.FilterType.HYDROGEN, Filter.FilterType.OXYGEN, Filter.FilterType.SULFUR]:
            Filter.objects.get_or_create(
                filter_type=filter_type,
        ...
        for filter_type in [Filter.FilterType.SR, Filter.FilterType.SG, Filter.FilterType.SI]:
            Filter.objects.get_or_create(
                filter_type=filter_type,
        ...
        for filter_type in [Filter.FilterType.LUMINANCE, Filter.FilterType.RED, Filter.FilterType.GREEN, Filter.FilterType.BLUE, Filter.FilterType.HYDROGEN, Filter.FilterType.OXYGEN, Filter.FilterType.SULFUR]:
