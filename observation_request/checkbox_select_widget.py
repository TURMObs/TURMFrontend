...
class TURMCheckboxSelectWidget(Widget):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        for q in self.queryset:
            out += (f'<div class="multiple_choice_div{" tooltip_div" if self.tooltip is not None else ""}"> <input type="checkbox" name="{name}" value="{q}" '
                    ...
                    f'{self._render_tooltip(f"id_{name}_{i}") if self.tooltip is not None else ""}</div>')
            i += 1
        out += '</div>'
